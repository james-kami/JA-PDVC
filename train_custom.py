...
from data.video_dataset_custom import PropSeqDataset, collate_fn
...
def train(opt):
    ...
    eval_score, eval_loss = evaluate(model, criterion, postprocessors, val_loader, result_json_path, logger=logger,
    ...
    while True:
        ...
        if (epoch % opt.save_checkpoint_every == 0) and (epoch >= opt.min_epoch_when_save):
            ...
            result_json_path = os.path.join(save_folder, 'prediction',
                                         'num{}_epoch{}.json'.format(
                                             len(val_dataset), epoch))
            print('out_path', result_json_path)
