...
def check_timestamps(data):
    ...
    for k, v in data.items():
        ...
        for pair in timestamps:
            if pair[0] > pair[1]:
                print(f"Error in {k}: start time {pair[0]} is greater than end time {pair[1]}")
                invalid_vid.append(k)
            else:
                print(f"{k}: Timestamps {pair[0]} to {pair[1]} are valid")
    return invalid_vid
...
class EDVCdataset(Dataset):
    ...
    def __init__(self, anno_file, feature_folder, translator_json, is_training, proposal_type, opt):
        ...
        invalid_vid = check_timestamps(self.anno)
        self.keys = list(set(self.anno.keys()) - set(invalid_vid))
...
def get_feats(key, vf_type, vf_folder, data_norm=False):
    ...
    if vf_type == 'c3d':
        ...
        feat_dim = 512
        # path = os.path.join(vf_folder, key[0:13] + '.npy')
        path = os.path.join(vf_folder, key[0:22] + '.npy')
    elif vf_type == 'vggish':
        feat_dim = 128
        path = os.path.join(vf_folder, key[0:13] + '.npy')
    else:
